...
import { acornParser } from '../acorn/acornParser';
...
export const compileProjectWithAcorn = (state: ICompilerState): ICompilerState => {
	if (!state) {
		return undefined;
	}
	...
	for (let i = 0; i < sourceFileNames.length; i++) {
		try {
			...
			const compileResult = acornParser.parse(sourceFileContent);
